...
if meta:
    ...
    c1.metric("Base rows trained", f"{int(meta.get('rows_trained', meta.get('base_model_rows_trained', 0))):,}")
    c2.metric("Base F1", f"{float(meta.get('validation_f1', 0)):.3f}")
    c3.metric("Base precision", f"{float(meta.get('validation_precision', 0)):.3f}")
    c4.metric("Base recall", f"{float(meta.get('validation_recall', 0)):.3f}")
